...
import com.asmaulhusna.a99namesofallah.ads.AdmobNativeAds;
...
public class ActivityStart extends AppCompatActivity {
    ...
    private CardView admobMediumNative;
    ...
    class C02331 implements Runnable {
        ...
        public void run() {
            if (!ActivityStart.this.stop) {
                if (ActivityStart.this.audio.getDuration() >= ActivityStart.this.audio.getCurrentPosition()) {
                    ActivityStart.this.currentTime = ActivityStart.this.audio.getCurrentPosition();
                    if (ActivityStart.this.audio.getCurrentPosition() >= ActivityStart.this.timeDelay[99]) {
                        ActivityStart.this.picture.setBackgroundResource(ActivityStart.this.nameId[98]);
                        ActivityStart.this.Name.setText(ActivityStart.this.allData[196]);
                        ActivityStart.this.meaning.setText(ActivityStart.this.allData[197]);
                    } else {
                        if (ActivityStart.this.picture.getLayoutParams().height < 550) {
                            ActivityStart.this.time.setText(Integer.toString(ActivityStart.this.picture.getLayoutParams().height));
                            ActivityStart.this.picture.getLayoutParams().height += 50;
                            ActivityStart.this.picture.getLayoutParams().width += 50;
                        }
                        changeImage(ActivityStart.this.currentTime);
                    }
                    ActivityStart.this.handler.postDelayed(this, 0);
                    return;
                }
                ActivityStart.this.startActivity(new Intent(ActivityStart.this, MainActivity.class));
            }
        }

        private void changeImage(int currentTime) {
            if (ActivityStart.this.f49j >= 99) {
                return;
            }
            if (currentTime < ActivityStart.this.timeDelay[ActivityStart.this.f49j + 1]) {
                if (ActivityStart.this.f49j == 98) {
                }
                ActivityStart.this.picture.setBackgroundResource(ActivityStart.this.nameId[ActivityStart.this.f49j]);
                ActivityStart.this.Name.setText(ActivityStart.this.allData[ActivityStart.this.f48i]);
                ActivityStart.this.meaning.setText(ActivityStart.this.allData[ActivityStart.this.f48i + 1]);
                return;
            }
            ActivityStart.this.picture.getLayoutParams().height = 0;
            ActivityStart.this.picture.getLayoutParams().width = 0;
            ActivityStart start = ActivityStart.this;
            start.f49j++;
            start = ActivityStart.this;
            start.f48i += 2;
        }
    }
    ...
    class C02342 implements OnClickListener {
        ...
        public void onClick(View v) {
            ActivityStart.this.audio.seekTo(ActivityStart.this.audio.getCurrentPosition() + 20000);
        }
    }
    ...
    class C02353 implements OnClickListener {
        ...
        public void onClick(View arg0) {
            if (ActivityStart.this.audio.isPlaying()) {
                ActivityStart.this.audio.pause();
                return;
            }
            ActivityStart.this.audio.start();
            ActivityStart.this.handler.postDelayed(this.val$checkTime, 0);
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nativelay=findViewById(R.id.largenativelay);
    ...
    private void load_native_ad() {
        AdmobNativeAds.loadNativeAd(this, admobMediumNative,
                1, getString(R.string.native_advance),
                NativeAdOptions.ADCHOICES_TOP_LEFT, new NativeAd.OnNativeAdLoadedListener() {
                    @Override
                    public void onNativeAdLoaded(@NonNull NativeAd nativeAd) {
                        nativeAdView = (NativeAdView) getLayoutInflater().inflate(R.layout.admob_medium_native, null);
                        if (nativeAdView != null) {
                            AdmobNativeAds.inflateNativeAd_with_media(nativeAd, nativeAdView);
                            admobMediumNative.removeAllViews();
                            admobMediumNative.addView(nativeAdView);
                        }
                    }
                });
    }
